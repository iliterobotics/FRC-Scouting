...
module.exports = {
    ...
    simboticsDataImport: function (file, division) {
        ...
        while (teamRow) {
            ...
            for (var stackIndex = 0; stackIndex < normalizedNumStacks; stackIndex++) {
                stacksList.push({
                    height: avgStackHeight.toFixed(2),
                    location: 'HP'
                });
                capsList.push({
                    height: avgStackHeight.toFixed(2),
                    litter: avgLitterScore / 6
                });
            }
    ...
    matchImport: function (file, type, gameId) {
        if (type === 'xlsx') {
            ...
            while (matchRow) {
                ...
                Match.saveMatch(gameId, matchToAdd, function (err) {
                    if (err) {
                        console.log(err);
                    }
                });
